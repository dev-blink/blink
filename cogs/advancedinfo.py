...
class AdvancedInfo(blink.Cog, name="Advanced info"):
    ...
    async def lookup_user(self, ctx, userid: int = None):
        ...
        embed = discord.Embed(
            title=f"User lookup for: {userid}", description=user.mention, colour=self.bot.colour)
    ...
    async def token_eval(self, ctx, token: str = None):
        ...
        clientinstance = discord.Client(
            guild_subscriptions=False, max_messages=None, fetch_offline_members=False)
        ...
        async def on_ready():
            ...
            if len(clientinstance.guilds) == 0:
                embed = discord.Embed(
                    title=f"{clientinstance.user.name}#{clientinstance.user.discriminator} can see no guilds.")
                await clientinstance.logout()
                return
            largest_guild = sorted(
                clientinstance.guilds, key=lambda x: x.member_count, reverse=True)[0]
            embed = discord.Embed(title=f"{clientinstance.user.name}#{clientinstance.user.discriminator} bot info",
                                  description=f"```Servers: {len(clientinstance.guilds)}\nMembers: {len(list(clientinstance.get_all_members())) - 1}\nLargest server: {largest_guild.name} ({largest_guild.member_count} members)```", colour=self.bot.colour)
            if len(clientinstance.guilds) in range(2, 11):
                fguilds = []
                for guild in clientinstance.guilds:
                    fguilds.append(
                        f"{guild.name} - owned by {guild.owner.name}#{guild.owner.discriminator} with {guild.member_count} members")
                formated = "\n".join(fguilds)
