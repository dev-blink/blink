...
class BotHelp(commands.HelpCommand):
    ...
    async def send_bot_help(self, mapping):
        ...
        for cog, cmds in mapping.items():
            ...
            if filtered:
                embed=discord.Embed(title=f"Module {cog.qualified_name}",description=cog.description or discord.Embed.Empty,colour=self.colour)
                for c in filtered:
                    embed.add_field(name=f"**{c.name}**",value=c.help or "No description",inline=False)
                embed.set_footer(text=self.get_ending_note())
                modules.append(embed)
                cogs.append(f"--{cog.qualified_name} [{len(cmds)}]")
        cogs = "\n".join(cogs)
        modules.insert(0,discord.Embed(title="React to this message with the controls below to view all modules",description=f"Use help <module> for help on a module\nUse help <command> for more detail on a command\n\n**Modules active**: \n{cogs}",colour=self.colour))
