...
class Player(wavelink.Player):
    ...
    def build_embed(self) -> typing.Optional[discord.Embed]:
        ...
        embed.set_footer(
            text=f"Played by: {track.requester.nick or track.requester.name}")
        return embed
...
class Music(blink.Cog):
    ...
    async def play(self, ctx: commands.Context, *, query: str):
        ...
        if isinstance(tracks, wavelink.TrackPlaylist):
            ...
            embed = discord.Embed(
                title=track.title, url=track.uri, colour=self.bot.colour)
            embed.add_field(name="**Duration**",
                            value=blink.prettydelta(track.duration // 1000))
            embed.add_field(name="**Queue position**",
                            value=f"{player.queue.qsize() + 1}")
            embed.add_field(name="**Added by**", value=ctx.author.mention)
            embed.set_image(
                url=f"https://img.youtube.com/vi/{track.uri[32:]}/maxresdefault.jpg")
